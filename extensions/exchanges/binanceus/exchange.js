...
module.exports = function binanceus(conf) {
  ...
  function publicClient() {
    if (!public_client)
      public_client = new ccxt.binanceus({
        apiKey: "",
        secret: "",
  ...
  function authedClient() {
    if (!authed_client) {
      if (
        !conf.binanceus ||
        !conf.binanceus.key ||
        conf.binanceus.key === "YOUR-API-KEY"
      ) {
        throw new Error(
          "please configure your BinanceUS credentials in " +
            path.resolve(__dirname, "conf.js")
        );
      }
      authed_client = new ccxt.binanceus({
        apiKey: conf.binanceus.key,
        secret: conf.binanceus.secret,
        options: { adjustForTimeDifference: true },
  ...
  function retry(method, args, err) {
    if (method !== "getTrades") {
      console.error(
        (
          "\nBinanceUS API is down! unable to call " +
          method +
          ", retrying in 20s"
        ).red
      );
