...
module.exports = function (program, conf) {
  program
    ...
    .option(
      "--start <unix_in_ms> | <datetime>",
      'lower bound as unix time in ms or format ("YYYYMMDDhhmm")',
      Number,
      -1
    )
    .option(
      "--end   <unix_in_ms> | <datetime>",
      'upper bound as unix time in ms or format ("YYYYMMDDhhmm")',
      Number,
      -1
    )
    .action(function (selector, cmd) {
      ...
      var exchange =
        require(`../extensions/exchanges/${selector.exchange_id}/exchange`)(
          conf
        );
      if (!exchange) {
        console.error(
      ...
      if (mode === "backward") {
        ...
        if (cmd.start >= 0 && cmd.end >= 0) {
          if (
            moment(cmd.start, "YYYYMMDDhhmm").isValid() &&
            moment(cmd.end, "YYYYMMDDhhmm").isValid()
          ) {
